...
export function SignupPage() {
  ...
  const canSubmit = !!username && !!email && pw.length >= 6 && !mismatch && !invalidUser && !loading;
  ...
  return (
    <AuthLayout title="Create account" subtitle="Join Social Spark — it takes 30 seconds.">
      <Box component="form" onSubmit={submit}>
        <Stack spacing={2}>
          <TextField label="Username" value={username} onChange={(e) => setUsername(e.target.value.trim())}
            required error={invalidUser} helperText={invalidUser ? "3–20 chars, letters/numbers/_." : " "} />
          <TextField label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <Box>
            <TextField label="Password" type={show ? "text" : "password"} value={pw}
              onChange={(e) => setPw(e.target.value)} required fullWidth
              InputProps={{
                endAdornment: (
            ...
            {pw && (
              <Box sx={{ mt: 1 }}>
                <LinearProgress variant="determinate" value={(strength.score / 4) * 100}
                  sx={{ height: 6, borderRadius: 999, bgcolor: "action.hover", "& .MuiLinearProgress-bar": { bgcolor: strength.color } }} />
                <Typography variant="caption" sx={{ color: strength.color, fontWeight: 700 }}>{strength.label}</Typography>
              </Box>
            )}
          </Box>
          <TextField label="Confirm password" type={show ? "text" : "password"} value={confirm}
            onChange={(e) => setConfirm(e.target.value)} required error={mismatch}
          ...
          <Typography variant="body2" color="text.secondary" align="center">
            ...
            <Link to="/login" style={{ color: "inherit", fontWeight: 700 }}>Sign in</Link>
          </Typography>
        </Stack>
      </Box>
    </AuthLayout>
